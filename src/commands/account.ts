...
import { toAccountToon } from "../lib/mappers/account.js";
...
export async function accountCommand(args: string[], ctx?: DoctlContext): Promise<string> {
  const sub = args[0];
  ...
  if (sub === "--help" || sub === "-h") return ACCOUNT_HELP;
  if (sub === "get") return accountGet(args.slice(1), ctx);
  throw new AxiError(`Unknown subcommand: ${sub}`, "VALIDATION_ERROR", ["Available: get", "Run `doctl-axi account --help`"]);
}

async function accountGet(rawArgs: string[], ctx?: DoctlContext): Promise<string> {
  if (rawArgs.includes("--help") || rawArgs.includes("-h")) return ACCOUNT_HELP;
  rejectUnknownFlags(rawArgs, ["--full"], "Run `doctl-axi account get --help`");
  const args = [...rawArgs];
