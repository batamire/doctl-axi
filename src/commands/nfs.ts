...
import { toNfsToon } from "../lib/mappers/nfs.js";
...
export async function nfsCommand(args: string[], ctx?: DoctlContext): Promise<string> {
  const sub = args[0];
  ...
  if (sub === "--help" || sub === "-h") return NFS_HELP;
  if (sub === "list") return nfsList(args.slice(1), ctx);
  if (sub === "get") return nfsGet(args.slice(1), ctx);
  throw new AxiError(`Unknown subcommand: ${sub}`, "VALIDATION_ERROR", ["Available: list, get", "Run `doctl-axi nfs --help`"]);
}

async function nfsList(rawArgs: string[], ctx?: DoctlContext): Promise<string> {
  ...
  const raw = await doctlJson<unknown>(["nfs", "list"], ctx?.context);
  ...
  const payload: Record<string, unknown> = {
    ...
    nfs: filtered,
    help: [suggest(ctx, `nfs get ${mapped[0].id}`, "for detail"), suggest(ctx, "nfs list --full", "for complete fields")],
  };
  return encode(payload);
}

async function nfsGet(rawArgs: string[], ctx?: DoctlContext): Promise<string> {
  if (rawArgs.includes("--help") || rawArgs.includes("-h")) return NFS_HELP;
  const args = [...rawArgs];
