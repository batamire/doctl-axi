  ...
  if (idx !== -1) {
    ...
    if (val === undefined || val.startsWith("-")) {
      throw new AxiError(`Flag ${flag} requires a value`, "VALIDATION_ERROR", [
        `Run \`do-axi docs --help\` for available flags`,
      ]);
    }
    args.splice(idx, 2);
    return val;
...
export const DOCS_HELP = encode({
  ...
  usage: "do-axi docs <subcommand> [args] [flags]",
  subcommands: {
    search: "Search docs via llms.txt — do-axi docs search <query>",
    get: "Fetch a docs page — do-axi docs get <path>",
    "find-for-service": "Find docs for a service — do-axi docs find-for-service <service>",
    "get-quickstart": "Fetch quickstart guide — do-axi docs get-quickstart <path>",
    troubleshoot: "Fetch troubleshooting guide — do-axi docs troubleshoot <path>",
    "get-related": "Find related docs — do-axi docs get-related <path>",
  },
  flags: {
  ...
  examples: [
    ...
    "do-axi docs get /products/droplets/how-to/resize/",
    "do-axi docs search droplets --full",
    "do-axi docs find-for-service app-platform",
  ],
});
...
const PER_SUB_HELP: Record<string, string> = {
  search: encode({
    command: "docs search",
    usage: "do-axi docs search <query> [--full] [--fields path,title,excerpt]",
    ...
    examples: ['do-axi docs search "droplet resize"', "do-axi docs search droplets --full"],
  }),
  get: encode({
    command: "docs get",
    usage: "do-axi docs get <path> [--full] [--fields path,excerpt]",
    ...
    examples: ["do-axi docs get /products/droplets/how-to/resize/", "do-axi docs get /products/droplets/how-to/resize --full"],
  }),
  "find-for-service": encode({
    command: "docs find-for-service",
    usage: "do-axi docs find-for-service <service> [--full] [--fields path,title,excerpt]",
    ...
    examples: ["do-axi docs find-for-service app-platform"],
  }),
  "get-quickstart": encode({
    command: "docs get-quickstart",
    usage: "do-axi docs get-quickstart <path> [--full] [--fields path,excerpt]",
    ...
    examples: ["do-axi docs get-quickstart /products/droplets/"],
  }),
  troubleshoot: encode({
    command: "docs troubleshoot",
    usage: "do-axi docs troubleshoot <path> [--full] [--fields path,excerpt]",
    ...
    examples: ["do-axi docs troubleshoot /products/droplets/how-to/resize/"],
  }),
  "get-related": encode({
    command: "docs get-related",
    usage: "do-axi docs get-related <path> [--full] [--fields path,title,excerpt]",
    ...
    examples: ["do-axi docs get-related /products/droplets/how-to/resize/"],
  }),
};
...
export async function docsCommand(args: string[], _context: unknown): Promise<string> {
  ...
  if (!sub || !SUBCOMMANDS.has(sub)) {
    if (sub) {
      throw new AxiError(`Unknown subcommand: ${sub}`, "VALIDATION_ERROR", [
        ...
        "Run `do-axi docs --help` for usage",
      ]);
    }
    return DOCS_HELP;
  }
  ...
  if (leftoverFlags.length > 0) {
    throw new AxiError(`Unknown flag: ${leftoverFlags[0]}`, "VALIDATION_ERROR", [
      `Run \`do-axi docs ${sub} --help\` for available flags`,
    ]);
  }
...
async function handleGet(args: string[], full: boolean, fieldsArg: string | undefined): Promise<string> {
  ...
  if (args.length === 0) {
    throw new AxiError("Missing required argument: <path>", "VALIDATION_ERROR", [
      "Usage: do-axi docs get <path> [--full]",
    ]);
  }
  const docPath = args[0]!.trim();
  if (args.length > 1) {
    throw new AxiError(`Unexpected argument: ${args[1]}`, "VALIDATION_ERROR", ["Run `do-axi docs get --help`"]);
  }
...
async function handleFindForService(args: string[], full: boolean, fieldsArg: string | undefined): Promise<string> {
  ...
  if (args.length === 0) {
    throw new AxiError("Missing required argument: <service>", "VALIDATION_ERROR", [
      "Usage: do-axi docs find-for-service <service>",
    ]);
  }
...
async function handleGetQuickstart(args: string[], full: boolean, fieldsArg: string | undefined): Promise<string> {
  ...
  if (args.length === 0) {
    throw new AxiError("Missing required argument: <path>", "VALIDATION_ERROR", [
      "Usage: do-axi docs get-quickstart <path>",
    ]);
  }
  const docPath = args[0]!.trim();
  if (args.length > 1) {
...
async function handleTroubleshoot(args: string[], full: boolean, fieldsArg: string | undefined): Promise<string> {
  if (args.length === 0) {
    throw new AxiError("Missing required argument: <path>", "VALIDATION_ERROR", [
      "Usage: do-axi docs troubleshoot <path>",
    ]);
  }
  const docPath = args[0]!.trim();
  if (args.length > 1) {
...
async function handleGetRelated(args: string[], full: boolean, fieldsArg: string | undefined): Promise<string> {
  ...
  if (args.length === 0) {
    throw new AxiError("Missing required argument: <path>", "VALIDATION_ERROR", [
      "Usage: do-axi docs get-related <path>",
    ]);
  }
