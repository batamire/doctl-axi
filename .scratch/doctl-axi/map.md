# Map: doctl-axi — single AXI replacing multiple DigitalOcean MCPs

## Destination

A shippable `doctl-axi` CLI (npm package `doctl-axi`, binary `doctl-axi`) that wraps `doctl` (+ direct REST only where `doctl` gaps) behind AXI's 10 principles (TOON, 3-4 fields/list, truncation+`--full`, aggregates, definitive empties, structured errors, ambient context, content-first dashboard, suggestions, `--help`), consolidating the 21 fragmented `digitalocean-labs/mcp-digitalocean` remote MCP services into one agent-ergonomic surface. Published to npm, listed in `kunchenguid/axi/catalog.yaml` (community), with `doctl-axi setup hooks` + skill for ambient context. The map is done when the spec + implementation plan is decided — command surface, TOON field map, doctl↔MCP coverage matrix, auth/dashboard contract, wrapper boundary, packaging/catalog requirements — nothing left to decide before build.
...
- [Teardown: axi-sdk-js + TOON + gh-axi / chrome-devtools-axi pattern](issues/02-axi-sdk-toon-reference-teardown.md) — gh-axi `execFile`+TOON pattern is template for doctl-axi; SDK gives runAxiCli/home/hooks, we build doctl.ts/format/toon/errors; 10MB buffer & stdin-secrets pitfalls captured.
...
- [Packaging, distribution & catalog](issues/09-packaging-distribution-catalog.md) — `doctl-axi` npm available, `axi-sdk-js@0.1.10`+`toon@2.1.0`, pnpm+Node>=20, fast-path bin, release-please, MIT standalone repo + catalog PR.
- [Foundation + Droplet slice](issues/10-foundation-droplet-slice.md) — runnable `doctl-axi droplet list` via `doctl --output json` → TOON `count/status` + 5 fields + `help:` + `0 droplets` + `--full`/`--fields` + `AUTH_MISSING`; auth shim flag>ACCESS_TOKEN>API_TOKEN>config.yaml>stdin (non-blocking) + 10MB buffer; CLI seam 10 tests green.
...
- [Dashboard + Hooks + Packaging slice](issues/16-dashboard-hooks-packaging-slice.md) — bare `doctl-axi` `Promise.allSettled` dashboard `account/balance` + 6 counts + `—` degradation + `help`; `setup hooks` idempotent + `--check` drift (codexConfigOk fix); `skills/doctl-axi` + `build-skill` + bin fast-path `--version`/`-v`; package `doctl-axi` Node>=20 pnpm catalog + `license MIT` + `release-please` + `CHANGELOG` + workflows, 131 tests.
