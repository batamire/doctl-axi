...
33. As an agent, I want `doctl-axi <noun> --help` concise per-command help, so that I can learn flags without full docs.
...
36. As an operator, I want bare `doctl-axi` to show a rich dashboard (`account` email/team, `balance`, plus counts for `droplet`/`app`/`database`/`kubernetes`/`registry`/`domain`) via parallel fetches, so that Principle 8 content-first holds and I see live state without args.
37. As an operator, I want `doctl-axi setup hooks` to install SessionStart hooks for Claude/Codex/OpenCode, so that ambient DO context appears every session (Principle 7).
...
40. As a maintainer, I want `doctl-axi update` and `release-please` versioning, so that upgrades are decoupled from hook/skill installs.
...
- **Packaging:** `name: doctl-axi`, bins `doctl-axi` + `doctl-axi` (bin alias required: npm exec resolves only bin==package name), `type: module`, `files: [dist, skills/doctl-axi, LICENSE, README.md]`, license MIT, standalone repo, community catalog entry in `kunchenguid/axi/catalog.yaml`.
