...
38. As an operator, I want `npx -y do-axi` zero-setup and `npx skills add do-axi` skill install, so that I can try without global install.
39. As an operator, I want gap nouns (`genai-batchinference`, `vector-database`, etc.) to return `NOT_IMPLEMENTED` with deferred hint and remote MCP URL, so that I know to use the hosted MCP until v2.
...
- **Distribution:** Skill (`skills/do-axi/SKILL.md`, `user-invocable:false`, Hermes devops metadata, `npx skills add do-axi`) is primary zero-setup path; `npm` global (`npm i -g do-axi` + `npx -y do-axi`) plus `do-axi setup hooks` (writes `~/.claude/settings.json`, `~/.codex/hooks.json`, opencode plugin) is secondary ambient path. Versioning via `release-please`, self-update via `do-axi update`.
- **Packaging:** `name: do-axi`, bin `do-axi`, `type: module`, `files: [dist, skills/do-axi, LICENSE, README.md]`, license MIT, standalone repo, community catalog entry in `kunchenguid/axi/catalog.yaml`.
...
- `genai-batchinference`, `genai-custom-models`, `genai-inferencerouter`, `genai-evaluation`, `vector-database`, `inference-modelcatalog` deep search, `function` atomic `create-or-update-action/package/trigger` with inline `Code` and `invoke` data-plane, and `database` online-migration `start/stop/get-migration` — deferred to v2 (return `NOT_IMPLEMENTED` with remote MCP hint until then).
